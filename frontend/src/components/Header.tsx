...
import {Clock3,LogOut,Search,ShoppingCart,UserRound} from 'lucide-react'
...
export default function Header(){
  ...
  return <>
    ...
    <header><div className="container nav"><Link className="brand" to="/"><span className="mark small">{site.brand_mark}</span>{site.brand_short}</Link><NavLink className="catalogBtn" to="/catalog">{site.catalog_label}</NavLink><div className="search"><Search size={17}/><span>Поиск по каталогу…</span></div><nav><Link to="/catalog">{site.catalog_label}</Link><Link to="/contacts">Контакты</Link>{user?<><Link to="/account"><UserRound size={18}/> {user.full_name?.split(' ')[0]||'Кабинет'}</Link><button className="navLogout" onClick={()=>session.logout()} title="Выйти"><LogOut size={18}/></button></>:<Link to="/login"><UserRound size={18}/> Войти</Link>}<span className="icon"><ShoppingCart size={20}/><i>0</i></span></nav></div></header>
  </>
}
